...
# transacts and grows the population. Amaru profiles intentionally omit
# this perturbator, so the gate is conditional on the service existing.
if docker compose --progress quiet -f "$COMPOSE_FILE" config --services | grep -Fxq tx-generator; then
  ...
  echo "Skipping tx-generator smoke gate: service absent from ${TESTNET}"
fi
