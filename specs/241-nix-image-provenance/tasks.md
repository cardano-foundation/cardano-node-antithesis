...
- [ ] T241-1 Apply the source-time policy: new `components/image-meta`
  helper; `created` + OCI labels in all six `nix/docker-image.nix`;
  `path:../image-meta` wiring in all six `flake.nix` (lock the new inputs).
- [ ] T241-2 Ship `scripts/check-image-provenance.sh` per the CLI contract;
...
- [ ] T241-3 Ship `tests/test-image-provenance.sh` per the proof contract,
...
- [ ] T241-4 Run the real packaging path for representative images
...
- [ ] T241-5 Document the timestamp policy, the operator build/publish
