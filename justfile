...
stop testnet='cardano_node_master':
    ...
    docker compose -f testnets/cardano_node_master/docker-compose.yaml down --volumes --remove-orphans
...
exec container testnet='cardano_node_master':
    ...
    docker compose -f testnets/cardano_node_master/docker-compose.yaml exec -it {{ container }} bash

logs container testnet='cardano_node_master':
    #!/usr/bin/env bash
    export INTERNAL_NETWORK=true
    echo "Tailing logs for container {{ container }}..."
    docker compose -f testnets/cardano_node_master/docker-compose.yaml logs -f {{ container }}

ps testnet='cardano_node_master':
    #!/usr/bin/env bash
    export INTERNAL_NETWORK=true
    echo "Listing containers for cardano_node_master testnet..."
    docker compose -f testnets/cardano_node_master/docker-compose.yaml ps

stats testnet='cardano_node_master':
    #!/usr/bin/env bash
    export INTERNAL_NETWORK=true
    echo "Showing stats for cardano_node_master testnet..."
    docker stats $(docker compose -f testnets/cardano_node_master/docker-compose.yaml ps -q)
